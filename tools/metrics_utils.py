...
def normalize(image):
    """
    Normalize the pixel values in the image to the range [0, 1].
    """
    norm_image = (image - np.min(image)) / (np.max(image) - np.min(image))
    return norm_image*255
...
def psnr(img1, img2):
    ...
    # return np.mean(p)
    mse = math.sqrt(np.mean((img1 - img2) ** 2))
    if mse == 0:
        PSNR = 100
    PSNR = 20 * math.log10(1 / mse)
    return PSNR
...
def rmse(img1, img2):
    ...
    #     return np.sqrt(mse)
    #
    # batches, channels, _, __ = real_hr.shape
    # rmse_sum = 0
    # for sample in range(batches):
    #     for band in range(channels):
    #         fake_band_img = fake_hr[sample, band, :, :]
    #         real_band_img = real_hr[sample, band, :, :]
