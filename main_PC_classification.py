...
if __name__ == "__main__":
    ...
    save_path = './model_PC_SR/'
    save_path_classifier = './model_PC_classifier/'
    load_path = './model_PC_SR/SR3_model_epoch-2.pth'
    save_path_predHR = './result/class_pc/predHR/'
    save_path_gtHR = './result/class_pc/gtHR/'
    ...
    for idx, (hrHS, lrHS, gt) in enumerate(test_loader):
        with torch.no_grad():
            ...
            b, c, h, w = hrHS.shape
            randn3 = np.random.randint(0, b)
            img1, result_sr = sr3.test(hrHS, lrHS, gt)
            ...
            # 测试点
            if np.count_nonzero(gt.cpu().detach().numpy()) != 0:  # 检查train_label是否有非零元素
                TrainPatch_lrHSI, TrainPatch_2X, TrainPatch_pred, TrainLabel = gen_cnn_data_hr(lrHS, img1,
                                                                                                 result_sr, patch_size,
                                                                                                 patch_size,
                                                                                                 patch_size,
                                                                                                 gt.cpu().detach().numpy())
                # 将每个批次的数据添加到相应的列表中
                all_TestPatch_lrHSI.append(TrainPatch_lrHSI)
                all_TestPatch_2X.append(TrainPatch_2X)
                all_TestPatch_pred.append(TrainPatch_pred)
                all_TestLabel.append(TrainLabel)
    ...
    # 使用torch.cat将每个列表中的所有批次数据堆叠起来
    all_TestPatch_lrHSI = torch.cat(all_TestPatch_lrHSI, dim=0)
    all_TestPatch_2X = torch.cat(all_TestPatch_2X, dim=0)
    all_TestPatch_pred = torch.cat(all_TestPatch_pred, dim=0)
    all_TestLabel = torch.cat(all_TestLabel, dim=0)
    torch.save(all_TestPatch_lrHSI, save_path_classification_data + 'all_TestPatch_lrHSI.pt')
    torch.save(all_TestPatch_2X, save_path_classification_data + 'all_TestPatch_2X.pt')
    torch.save(all_TestPatch_pred, save_path_classification_data + 'all_TestPatch_pred.pt')
    torch.save(all_TestLabel, save_path_classification_data + 'all_TestLabel.pt')


    print("all_TestPatch_lrHSI.shape:", all_TestPatch_lrHSI.shape, "all_TestPatch_2X.shape:", all_TestPatch_2X.shape,"all_TestPatch_pred.shape:", all_TestPatch_pred.shape, "all_TestLabel.shape:", all_TestLabel.shape)

    # 分类器 训练 

    # all_TestPatch_lrHSI = torch.load(save_path_classification_data + 'all_TestPatch_lrHSI.pt')
    # all_TestPatch_2X = torch.load(save_path_classification_data + 'all_TestPatch_2X.pt')
    # all_TestPatch_pred = torch.load(save_path_classification_data + 'all_TestPatch_pred.pt')
    # all_TestLabel = torch.load(save_path_classification_data + 'all_TestLabel.pt')

    test_dataset = torch.utils.data.TensorDataset(all_TestPatch_lrHSI, all_TestPatch_2X, all_TestPatch_pred,
                                                  all_TestLabel)
    test_class_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size_classifier, shuffle=False)
    ...
    for experiment in range(num_experiments):
        model_classification = myclassifier(channels=bands, num_classes=num_classes).to(device)
        criterion_classification = nn.CrossEntropyLoss()
        optimizer_classification = torch.optim.Adam(model_classification.parameters(), lr=lr)
        best_acc = -1000
        seed_train = seed + experiment
        train_lr, train_2x, train_pred, train_y = split_train_data_threshold_random(all_TestPatch_lrHSI,
                                                                                    all_TestPatch_2X,
                                                                                    all_TestPatch_pred,
                                                                                    all_TestLabel, train_num_classifier,
                                                                                    0, rand_state=seed_train)
        print("train_lr.shape:", train_lr.shape, "train_2x.shape:", train_2x.shape, "train_pred.shape:",
              train_pred.shape, "train_y.shape:", train_y.shape)
        train_dataset = torch.utils.data.TensorDataset(train_lr, train_2x, train_pred, train_y)
        train_class_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size_classifier,
                                                             shuffle=True)
        for epoch in range(epochs_classifier):
            model_classification.train()
            running_loss1 = np.ones((len(train_class_dataloader))) * 1000.0
            running_loss2 = np.ones((len(train_class_dataloader))) * 1000.0
            train_loss = np.ones((len(train_class_dataloader))) * 1000.0
            train_acc = np.ones((len(train_class_dataloader))) * -1000.0
            train_acc_4x = np.ones((len(train_class_dataloader))) * -1000.0

            for step, [TrainPatch_lrHSI, TrainPatch_2X, TrainPatch_pred, TrainLabel] in enumerate(train_class_dataloader):
                #
                TrainPatch_lrHSI = TrainPatch_lrHSI.to(device).type(torch.float32)
                TrainPatch_2X = TrainPatch_2X.to(device).type(torch.float32)
                TrainPatch_pred = TrainPatch_pred.to(device).type(torch.float32)
                TrainLabel = TrainLabel.to(device)

                optimizer_classification.zero_grad()
                output_4x, output_final = model_classification(TrainPatch_lrHSI, TrainPatch_2X, TrainPatch_pred)
                loss_1 = criterion_classification(output_4x, TrainLabel)
                loss_2 = criterion_classification(output_final, TrainLabel)
                loss = loss_1 + loss_2
                loss.backward()
                optimizer_classification.step()
                running_loss1[step] = loss_1.item()
                running_loss2[step] = loss_2.item()
                train_loss[step] = loss.item()
                progress_bar(step, len(train_class_dataloader), 'Loss: %.4f' % (np.average(train_loss)))

                train_acc[step] = accuracy(output_final.data, TrainLabel.data, topk=(1,))[0].item()
                train_acc_4x[step] = accuracy(output_4x.data, TrainLabel.data, topk=(1,))[0].item()

            print('损失函数:')
            x = PrettyTable()
            x.add_column("epoch", [epoch])
            x.add_column("loss", ['value'])
            x.add_column("loss_all", [np.average(train_loss)])
            x.add_column("loss_1", [np.average(running_loss1)])
            x.add_column("loss_2", [np.average(running_loss2)])
            x.add_column("acc", [np.average(train_acc)])
            x.add_column("acc_4x", [np.average(train_acc_4x)])
            print(x)

            # test
            model_classification.eval()
            test_running_loss1 = np.ones((len(test_class_dataloader))) * 1000.0
            test_running_loss2 = np.ones((len(test_class_dataloader))) * 1000.0
            testloss = np.ones((len(test_class_dataloader))) * 1000.0
            test_acc = np.ones((len(test_class_dataloader))) * -1000.0
            test_acc_4x = np.ones((len(test_class_dataloader))) * -1000.0

            with torch.no_grad():
                for step, [TestPatch_lrHSI, TestPatch_2X, TestPatch_pred, TestLabel] in enumerate(
                        test_class_dataloader):
                    #
                    TestPatch_lrHSI = TestPatch_lrHSI.to(device).type(torch.float32)
                    TestPatch_2X = TestPatch_2X.to(device).type(torch.float32)
                    TestPatch_pred = TestPatch_pred.to(device).type(torch.float32)
                    TestLabel = TestLabel.to(device)

                    output_4x, output_final = model_classification(TestPatch_lrHSI, TestPatch_2X, TestPatch_pred)
                    loss_3 = criterion_classification(output_4x, TestLabel)
                    loss_4 = criterion_classification(output_final, TestLabel)

                    test_acc[step] = accuracy(output_final.data, TestLabel.data, topk=(1,))[0].item()
                    test_acc_4x[step] = accuracy(output_4x.data, TestLabel.data, topk=(1,))[0].item()

                    test_running_loss1[step] = loss_3.item()
                    test_running_loss2[step] = loss_4.item()
                    testloss[step] = loss_3.item() + loss_4.item()
                    progress_bar(step, len(test_class_dataloader), 'Test_Loss: %.4f' % (np.average(testloss)))
            # 模型保存
            if best_acc <= np.average(test_acc):
                best_acc = np.average(test_acc)
                torch.save(model_classification.state_dict(), save_path_classifier + 'best_model_epoch.pth')
            print('test:')
            y = PrettyTable()
            y.add_column("epoch", [epoch])
            y.add_column("loss", ['value'])
            y.add_column("loss_all", [np.average(testloss)])
            y.add_column("loss_1", [np.average(test_running_loss1)])
            y.add_column("loss_2", [np.average(test_running_loss2)])
            y.add_column("test_acc", [np.average(test_acc)])
            y.add_column("test_acc_4x", [np.average(test_acc_4x)])
            print(y)

        # 加载模型
        print('begin test')
        model_classification.load_state_dict(torch.load(save_path_classifier + 'best_model_epoch.pth'))
        model_classification.eval()
        test_running_loss1 = np.ones((len(test_class_dataloader))) * 1000.0
        test_running_loss2 = np.ones((len(test_class_dataloader))) * 1000.0
        test_acc = np.ones((len(test_class_dataloader))) * -1000.0
        ...
        with torch.no_grad():
            for step, [TestPatch_lrHSI, TestPatch_2X, TestPatch_pred, TestLabel] in enumerate(
                ...
                output_4x, output_final = model_classification(TestPatch_lrHSI, TestPatch_2X, TestPatch_pred)
                [predicted.append(a) for a in output_final.data.cpu().numpy()]
                [gt.append(a) for a in TestLabel.data.cpu().numpy()]
                loss_3 = criterion_classification(output_4x, TestLabel)
                loss_4 = criterion_classification(output_final, TestLabel)
        ...
        classification, confusion, result = reports(
            np.argmax(np.array(predicted), axis=1),
            np.array(gt),
            'PC'
        )
        # 将结果添加到相应的列表中
    ...
    #
    #
    # print("Results Mean: ", results_mean)
    # print("Results Std: ", results_std)